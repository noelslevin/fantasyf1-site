...
<div class="tabs-content">
	<div class="content active" id="panel2-1">
		<table>
			...
			<tbody>
				<?php
				$sql = $dbh->prepare("SELECT forename, surname, teamname, SUM(race_points) AS points FROM `view_ff1allraceresults` WHERE Year = 2014 GROUP BY driverstoseasons_id ORDER BY points DESC LIMIT 5");
				$sql->execute();
	...
	<div class="content" id="panel2-2">
		<table>
			...
			<tbody>
				<?php
				$sql = $dbh->prepare("SELECT teamname, engine, SUM(race_points) AS points FROM `view_ff1allraceresults` WHERE Year = 2014 GROUP BY teamstoseasons_id ORDER BY points DESC Limit 5");
				$sql->execute();
				$row = $sql->fetchAll(PDO::FETCH_ASSOC);
				$totalrows = count($row);
				if ($sql->rowCount() > 0) {
...
<div class="tabs-content">
	<div class="content active" id="panel2-3">
		<table>
			...
			<tbody>
				<?php
				$sql = $dbh->prepare("SELECT username, team, points FROM `view_ff1individualstandingsbyyear` WHERE season = 2014 ORDER BY points DESC LIMIT 5");
				$sql->execute();
				$row = $sql->fetchAll(PDO::FETCH_ASSOC);
				$totalrows = count($row);
				if ($sql->rowCount() > 0) {
	...
	<div class="content" id="panel2-4">
		<table>
			...
			<tbody>
				<?php
				$sql = $dbh->prepare("SELECT team, points FROM `view_ff1teamstandingsbyyear` WHERE season = 2014 ORDER BY points DESC LIMIt 5");
				$sql->execute();
				$row = $sql->fetchAll(PDO::FETCH_ASSOC);
				$totalrows = count($row);
				if ($sql->rowCount() > 0) {
